...
⭐**重点**：该类任务的创新点主要聚焦于两个，一个是特征提取模块（即分别对HSI和SAR/LiDAR数据进行特征提取），另一个是特征融合模块（即对HSI和SAR/LiDAR特征进行融合）。简单架构可参考code/compare_net/cnnbasenet.py及code/compare_net/cnnbasenet.png，其中多尺度卷积模块为特征提取模块，concat为最简单的特征融合，两模块可任意替换成其他特征提取与特征融合模块。
