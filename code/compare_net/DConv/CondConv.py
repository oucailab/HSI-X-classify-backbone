...
class CondConvBlock(nn.Module):
    """
    A complete CondConv block that combines the routing function and CondConv2d.
    ...
    Args:
        ...
        padding (int or tuple, optional): Padding for convolution. Default: 0
        dilation (int or tuple, optional): Dilation rate. Default: 1
        groups (int, optional): Number of groups. Default: 1
        bias (bool, optional): Whether to use bias. Default: True
        num_experts (int, optional): Number of experts. Default: 3
    """
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True,
                 num_experts=3):
        ...
        self.routing_func = route_func(c_in=in_channels, num_experts=num_experts)
    ...
    def forward(self, x):
        routing_weights = self.routing_func(x)
        out = self.conv(x, routing_weights)
        return out